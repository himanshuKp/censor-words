...
"out of control", "help", "unhappy", "bad", "upset", "awful", "broken", "damage", 
"damaging", "dismal", "distressed", "distressed", "concerning", "horrible", "horribly", 
"questionable"]

punctuation = [",","|","?",",","%","/","(",")"]

def censor_three(input_text,censor_list,negative_words):
    input_text_words = []
    ...
    for i in range(0,len(input_text_words)):
        if(input_text_words[i] in censor_list) == True:
            word_clean = input_text_words[i]
            censord_words = ""
            for x in range(0,len(word_clean)):
                censord_words = censord_words + "X"
            input_text_words[i] = input_text_words[i].replace(word_clean,censord_words)
        count=0
        for i in range(0,len(input_text_words)):
